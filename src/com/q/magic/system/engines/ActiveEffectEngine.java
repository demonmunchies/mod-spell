...
public class ActiveEffectEngine
{
	...
	public synchronized void updateActiveEffects()
	{
		...
		{
			// Much like the tempEntity list, this one is so that active effects with 0 duration are removed.
			...
			for(ActiveEffect ae : e.getAeArr())
			{
				...
				if(ae.getDuration() > 0)
				{
					...
					// Adds the active effect back into the temp array to then be calculated next cycle
					tempEffect.add(ae);
				}
			...
			// Updates the entities' active effects list to only have the effects with durations left
			e.setAeArr(tempEffect);
			// If there are more active effects affecting the entity, the entity is added back into the array list of entites with active effects.
